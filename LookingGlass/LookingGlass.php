...
class LookingGlass
{
     ...
     *   True on success
     */
    public function ping($host, $count = 15)
    {
        if ($host = $this->validate($host)) {
     ...
     *   True on success
     */
    public function traceroute($host, $fail = 5)
    {
        if ($host = $this->validate($host)) {
            return $this->procExecute('traceroute -4 -w2 -m30 -q1', $host, $fail);
        }
        return false;
    }
...
/**
    ...
    {
        if ($host = $this->validate($host)) {
            return $this->procExecute('nmap, $host);
        }
        return false;
    }
